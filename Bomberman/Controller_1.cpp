	...
	for (int i = 0; i < m_board.boardInfo(LINES); i++)
	{
		for (int j = 0; j < m_board.boardInfo(COLUMNS); j++)
		{
			...
			case GUARD:
				...
				break;
			case GHOST:
				m_ghostIndex = (unsigned int) m_gameObjects.size();
			...
			case WALL:
				...
				break;
			case ROCK:
				m_rockIndex = (unsigned int)m_gameStaticObjects.size();
...
void Controller::run()
{
	...
	m_window = std::make_unique<sf::RenderWindow>(sf::VideoMode(800, 800), "Bomberman");
