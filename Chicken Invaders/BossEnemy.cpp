...
void BossEnemy::draw(sf::RenderWindow & window)
{
	...
	m_Sprite.setTextureRect(sf::IntRect(0, 0, 297, 333));
	m_Sprite.setOrigin(m_Sprite.getGlobalBounds().width / 2, m_Sprite.getGlobalBounds().height / 2);
	m_Sprite.setPosition(m_Sprite.getPosition());
	window.draw(m_Sprite);

	if (SpawnTime + ((m_bossFrame*2)*0.1f) <= Resources::get_instance().chrono.getElapsedTime().asSeconds()) {
		static int dir = 1;
		float angle = 10;
		
		if (m_Sprite.getRotation() >= 80 || m_Sprite.getRotation() <= 280) {
			dir *= -1;
		}
		m_bossFrame++;
		m_Sprite.rotate(dir*angle);
	}
}
//-----------------------------------------------------------------------------
